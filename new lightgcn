...
class Data:
    def __init__(self, path_to_data: str):
        ...
        self.train_data = self._load_ratings(f'{self.path}/train.txt')
        self.test_data = self._load_ratings(f'{self.path}/test.txt')

        if self.n_users == 0 or self.n_items == 0:
            raise ValueError("Nije moguće učitati podatke.")
    ...
    def _normalize_adj_matrix(self):
        ...
        return torch.sparse_coo_tensor(index, data, torch.Size(A_hat.shape))
...
class LightGCN(nn.Module):
    def __init__(self, config, adj_mat, layers, decay):
        ...
        self.embedding = nn.Embedding(self.n_nodes, config['embed_size'])
    ...
    def forward(self, users, pos_items=None, neg_items=None):
        ...
        final_embeddings = torch.mean(torch.stack(embs, dim=1), dim=1)
...
def evaluate_model(model, data_loader):
    ...
    for u in data_loader.test_data:
        ...
        if len(seen) > 0:
            seen_tensor = torch.LongTensor(seen).to(DEVICE)
